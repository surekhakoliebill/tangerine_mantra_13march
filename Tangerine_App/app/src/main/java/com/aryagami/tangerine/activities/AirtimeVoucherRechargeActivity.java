...
public class AirtimeVoucherRechargeActivity extends AppCompatActivity {
    Button findSubscriptionBtn;
    TextView msisdnText, nameText, statusText;
    TextInputEditText msisdnEditText;
    Activity activity = this;
    ProgressDialog progressDialog, progressDialog1;
    ListView vouchersList;
    ResellerVoucherType[] vouchersArray, airtimeVouchersArray;
    List<ResellerVoucherType> airtimeVoucherList = new ArrayList<ResellerVoucherType>();
    String MSISDN = "";
    LinearLayout subscriberLayout, listviewLayout;
    TextView headerName, voucherTypeList;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        headerName.setText("Airtime Voucher Recharge");
        voucherTypeList.setText("Airtime Vouchers List");
   ...
   /* private void getCurrentLocation() {
    ...
    protected void onResume() {
        ...
        getLocation();
       // locationCallback();
        CheckNetworkConnection.cehckNetwork(activity);
    }
            ...
            serviceHandler.getListVoucherTypes(new RestServiceHandler.Callback() {
                ...
                public void success(DataModel.DataType type, List<DataModel> data) {
                    ...
                    if (data != null | data.size() != 0) {
                        for (DataModel voucherType : data) {
                    ...
                    if (vouchersArray.length != 0) {
                        ...
                        for (ResellerVoucherType vo : vouchersArray) {
                            if (vo.voucherDescription.contains("Airtime")| vo.voucherDescription.contains("airtime")) {
                                airtimeVoucherList.add(vo);
                            }else{
                                continue;
                            }
                        }
                        airtimeVouchersArray = new ResellerVoucherType[airtimeVoucherList.size()];
                        airtimeVoucherList.toArray(airtimeVouchersArray);
                        if (airtimeVouchersArray.length != 0) {
                            listviewLayout.setVisibility(View.VISIBLE);
                            ArrayAdapter adapter = new ResellerTopUpVouchersListAdapter(activity, airtimeVouchersArray, MSISDN,"Airtime",coordinates);
                            vouchersList.setAdapter(adapter);
                        } else {
                            MyToast.makeMyToast(activity, "Airtime Vouchers are Not Found.", Toast.LENGTH_SHORT);
                        }
                    } else {
