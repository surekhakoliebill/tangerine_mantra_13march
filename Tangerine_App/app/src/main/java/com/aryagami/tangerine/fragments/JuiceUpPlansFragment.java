...
public class JuiceUpPlansFragment extends Fragment {
   ...
   /* private void showBundlePlans() {
        if( RegistrationData.getBundleList() != null){
            ...
            {
                if(plan.groupType.equals("Juice Up") && plan.enableVoucherChannel) {
    ...
    private void setPlanDetailsInList(List<PlanGroup> planGroups) {
        ...
        if (planGroupsList.size() != 0){
            ...
            MyToast.makeMyToast(getActivity(), "Juiceup plans data empty...", Toast.LENGTH_SHORT);
        }
    }
}
