...
public class ResellerETopupRequestsActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        headText.setText("E-Topup Requests");
        getEtopupList();
    ...
    private void getEtopupList() {
        ...
        try {
            ...
            serviceHandler.getETopupRequestsFromReseller(resellerId, new RestServiceHandler.Callback() {
                @Override
                public void success(DataModel.DataType type, List<DataModel> data) {
                    WalletAccountTransactionLogVo logVo = (WalletAccountTransactionLogVo)data.get(0);
                    if(logVo != null){
                        if(logVo.status.equals("success")){
                            if(logVo.walletRequests != null)
                            if(logVo.walletRequests.size() !=0){
                                transactionLogVos = new WalletAccountTransactionLogVo[logVo.walletRequests.size()];
                                logVo.walletRequests.toArray(transactionLogVos);

                               // ArrayAdapter
                                ArrayAdapter adapter = new ResellerETopupRequestsListAdapter(activity, transactionLogVos);
